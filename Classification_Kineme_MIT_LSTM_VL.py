...
def ks_encoding(kineme_seq, nKineme):
    ...
    encoded_features = np.asarray([np.asarray(onehot_encoding(kineme_seq[i], nKineme)) for i in range(m)])
...
#pass the label name and chunk size to the training_lstm_video method
